...
# my_classifier = KNeighborsClassifier() #define the model
# my_classifier.fit(x_train, y_train) #fit the model on the training data
```
...
```{python}
# NOTE: because y was the string column 'airport_code', the DecisionTreeClassifier
# was trained on string labels and already predicts strings ('ATL', 'DEN', ...).
# This remap is therefore a no-op left in for reference: replace() leaves any value
# it doesn't find ('ATL', etc.) unchanged. It only does real work if your model was
# trained on integer-encoded labels (0-6).
pred_df = pl.DataFrame(pred, schema=['airport_code'])
pred_df = pred_df.with_columns(
    pl.col('airport_code').cast(pl.String).replace({
        '0': 'ATL',
        '1': 'DEN',
        '2': 'IAD',
        '3': 'ORD',
        ...
        '5': 'SFO',
        '6': 'SLC'
    }).alias('airport_code_str')
)
pred = pred_df.select('airport_code_str').to_numpy().flatten()
```
...
print(classification_report(y_test_array, pred))
```
...
# This is what we did today, before this point it was last class

```{python}
from sklearn.metrics import confusion_matrix
print(confusion_matrix(y_test_array, pred))
...
result = permutation_importance(my_classifier, x_test, y_test_array, n_repeats=10, random_state=42)
...
# 4.3 New stuff for today

plot all of the importances 

```{python}
importances = pl.DataFrame({
    "xgb_importance": result.importances_mean,
    "decision_tree_importance": my_classifier.feature_importances_,
    "vars": x_train.columns
}).sort("xgb_importance", descending=True)
...
```{python}
imp_unpivot = importances.unpivot(
    index="vars",
    variable_name="model",
...
```{python}
(
    ggplot(data=imp_unpivot, mapping=aes(x="importance", y="vars", fill="model"))
    + geom_bar(stat="identity", position="dodge")
    + labs(title="permutation importance comparison")
